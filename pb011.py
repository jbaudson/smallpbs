...
# numbers in the same direction (up, down,
# left, right, or diagonally) in the 20×20 grid?

tab = ["08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08".split(),
...
def productMax(tab, len) :
    ...
    while i < 20 :
        ...
        while j < 20 :
            #line Product
            if j > 2 and maxProduct < int(tab[i][j]) * int(tab[i][j - 1]) * int(tab[i][j - 2]) * int(tab[i][j - 3]) :
                maxProduct = int(tab[i][j]) * int(tab[i][j - 1]) * int(tab[i][j - 2]) * int(tab[i][j - 3])
            if j < 20 - 3 and maxProduct < int(tab[i][j]) * int(tab[i][j + 1]) * int(tab[i][j + 2]) * int(tab[i][j + 3]) :
                maxProduct = int(tab[i][j]) * int(tab[i][j + 1]) * int(tab[i][j + 2]) * int(tab[i][j + 3])
            #column Product
            if i > 2 and maxProduct < int(tab[i][j]) * int(tab[i - 1][j]) * int(tab[i - 2][j]) * int(tab[i - 3][j]) :
                maxProduct = int(tab[i][j]) * int(tab[i - 1][j]) * int(tab[i - 2][j]) * int(tab[i - 3][j])
            if i < 20 - 3 and maxProduct < int(tab[i][j]) * int(tab[i + 1][j]) * int(tab[i + 2][j]) * int(tab[i + 3][j]) :
                maxProduct = int(tab[i][j]) * int(tab[i + 1][j]) * int(tab[i + 2][j]) * int(tab[i + 3][j])
            #diag Product NW-SE
            if (i > 2 and j > 2) and maxProduct < int(tab[i][j]) * int(tab[i - 1][j - 1]) * int(tab[i - 2][j - 2]) * int(tab[i - 3][j - 3]) :
                maxProduct = int(tab[i][j]) * int(tab[i - 1][j - 1]) * int(tab[i - 2][j - 2]) * int(tab[i - 3][j - 3])
            if i < 20 - 3 and j < 20 - 3 and maxProduct < int(tab[i][j]) * int(tab[i + 1][j + 1]) * int(tab[i + 2][j + 2]) * int(tab[i + 3][j + 3]) :
                maxProduct = int(tab[i][j]) * int(tab[i + 1][j + 1]) * int(tab[i + 2][j + 2]) * int(tab[i + 3][j + 3])
            #diag NE-SW
            if i > 2 and j < 20 - 3 and maxProduct < int(tab[i][j]) * int(tab[i - 1][j + 1]) * int(tab[i - 2][j + 2]) * int(tab[i - 3][j + 3]) :
                maxProduct = int(tab[i][j]) * int(tab[i - 1][j + 1]) * int(tab[i - 2][j + 2]) * int(tab[i - 3][j + 3])
            if (i < 20 - 3 and j > 2) and maxProduct < int(tab[i][j]) * int(tab[i + 1][j - 1]) * int(tab[i + 2][j - 2]) * int(tab[i + 3][j - 3]) :
                maxProduct = int(tab[i][j]) * int(tab[i + 1][j - 1]) * int(tab[i + 2][j - 2]) * int(tab[i + 3][j - 3])
            j += 1
        i += 1
    return (maxProduct)

print(productMax(tab, 4))
